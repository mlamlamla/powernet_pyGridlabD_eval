    ...
    def clear_trivial(self,dQ,dP) :
        ...
        if isDZero is False :
            ...
            for n,m in enumerate(D) :
                #print(Dq)
                Dq = append(append(Dq, Dq[-1]),float(m[1])+Dq[-1])
        ...
        while ( i < nb ) and ( j < ns ) and ( float(D[i][0]) >= S[j][0] ) : #loop until Price demand/supply is >= 1
            if float(D[i][1]) > S[j][1] : #Quantity Demanded > Quantity Selling
                Q = S[j][1]
                a = b = float(D[i][0])
                j = j+1
                v = 0
            elif float(D[i][1]) < S[j][1] : #Quantity Buying < Quantity Selling
                Q = float(D[i][1])
                a = b = S[j][0]
                i = i+1
                v = 0
