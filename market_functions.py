...
import gridlabd
import pandas
#from HH_global import flexible_houses, C, p_max, interval, city, prec, ref_price, price_intervals, load_forecast, unresp_factor, month, which_price
...
def create_market(df_WS,df_prices,p_max,prec,price_intervals,dt_sim_time):
    ...
    #forward prices
        ...
        minutes = price_intervals*interval
...
class Market :
    ...
    def buy(self,quantity,price=[],response=0.0,active=0.0,appliance_name=None) :
        ...
        if price == [] :
            price = self.Pmax*2 #unresponsive loads bid double the max price
...
#                self.Pmax = P_all.max()
    ...
    def plot(self,directory,dt='') :
        ...
        if self.Dp[1] == self.Dp[2] == 2*self.Pmax:
            ax.plot(self.Dq[2:],self.Dp[2:],'b')
            ax.plot(self.Sq,self.Sp,'r')
        else:
            ax.plot(self.Dq,self.Dp,'b')
            ax.plot(self.Sq,self.Sp,'r')
        if self.Qd == self.Qs and self.Pd == self.Ps :
            ax.plot(self.Qd,self.Pd,'ok')
        else :
            ax.plot(self.Qd,self.Pd,'ob')
            ax.plot(self.Qs,self.Ps,'or')
        
        ax.set_xlabel(self.Qlabel)
        ax.set_xlim(left=0)

        ax.set_ylabel(self.Plabel)
        ax.set_ylim([min(min(self.Dp)*1.1,0.0),self.Pmax*1.1])

        ppt.grid('on')
        fig.show()
        ppt.savefig(directory+'/market_'+str(dt)+'.png', bbox_inches='tight')
        if self.status > 0 :
            print('Market {}: Error {}'.format(self.name, self.status))
        elif dt != '' :
            print('Market {}: {}'.format(self.name,dt))
