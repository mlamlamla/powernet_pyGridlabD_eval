...
df_cost['abs_change_1600'] = df_cost['abs_change_1600']/18 #yearly savings
df_cost['abs_change_1600_riskprem5'] = df_cost['abs_change_1600_riskprem5']/18 #yearly savings
xlabel = 'Estimated yearly absolute cost change [USD]'
xlabel = 'Estimated daily absolute cost change [USD]'

ax = df_cost['abs_change_1600'].hist(bins=20,histtype='step',label='C = 1.6 MW, no risk premium',normed=True,linewidth=2,color='orange')
ax.vlines(df_cost['abs_change_1600'].median(),0.,ymax,color='orange',linestyles='dashed') #dashes=[5, 5])
...
fig.savefig(run+'/hist_prices_procneutral_PV_day.png', bbox_inches='tight',dpi=300)
