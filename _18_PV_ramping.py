...
df_system['flex_pv_t-1'] = df_system['flex_pv'].shift(interval)
df_system = df_system.iloc[interval:]
df_system['PV_ramp'] = (df_system['flex_pv_t-1'] - df_system['flex_pv'])*1000 #to kW

#Figure
xlabel = 'Ramp [kW]'
#ymax = 25.
ax = df_system['PV_ramp'].hist(bins=20,histtype='step',normed=True,linewidth=2,color='orange')
...
fig.savefig(run+'/'+run + '_' + ind + '_vis/' +'_18_ramping_PV_'+ind+'_'+str(interval)+'min.png', bbox_inches='tight',dpi=300)
