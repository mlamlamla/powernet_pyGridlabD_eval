...
inds = ['0010','0018','0048'] #560 flex houses
inds = ['0013','0033','0049'] #1120 flex houses
...
for ind in inds:
	...
	perc = df_settings.loc[(df_settings['run'] == run) & (df_settings['id'] == int(ind))]['unresp_factor'].iloc[0]
	print(ind+' with C = '+str(C))
	if lns:
		lns = lns + ax.plot(df_system.index,df_system['measured_real_power'],label=str(C)+' MW, safety factor '+str(perc))
...
perc = df_settings.loc[(df_settings['run'] == run) & (df_settings['id'] == int(ind))]['unresp_factor'].iloc[0]
print(ind+' with C = '+str(C))
lns = lns + ax.plot(df_system.index,df_system['measured_real_power'],label=str(C)+' MW, perfect forecast')
ax.plot(df_system.index,[C]*len(df_system.index),'r',dashes=[5,5])
df_system_C = df_system.loc[df_system['measured_real_power'] > C]
print('max: '+str(df_system_C['measured_real_power'].iloc[0]))
print('no of times C was exceeded: '+str(len(df_system_C.index)))
...
ppt.savefig(run+'/07_safetymargin_loadcurve_1120_perfforecast.png', bbox_inches='tight')
